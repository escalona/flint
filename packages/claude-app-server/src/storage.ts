 ...
 * Stores threads in ~/.flint/claude-app-server/threads/ as append-only JSONL rollouts.
 */
...
let STORAGE_DIR = join(homedir(), ".flint", "claude-app-server");
...
export class Storage {
  ...
  async loadThread(threadId: string): Promise<ThreadData | null> {
    ...
    const fromRollout = await this.loadLatestSnapshot(rolloutPath);
    if (fromRollout) {
      return fromRollout;
    }
  ...
  async listThreads(
    ...
    for (const id of entries) {
      ...
      if (data) {
        // Filter by archived if specified (archived means source === "archived")
  ...
  async setSdkSessionId(threadId: string, sdkSessionId: string): Promise<boolean> {
    const data = await this.loadThread(threadId);
    if (!data) return false;

    data.sdkSessionId = sdkSessionId;
    await this.saveThread(data);
    return true;
  }

  async getSdkSessionId(threadId: string): Promise<string | undefined> {
    const data = await this.loadThread(threadId);
    return data?.sdkSessionId;
  }
}
...
export function resetStorageDirectory(): void {
  STORAGE_DIR = join(homedir(), ".flint", "claude-app-server");
